...
G4VPhysicalVolume* A2SolidTarget::Construct(G4LogicalVolume *MotherLogic, G4double Z0){
  ...
  G4double zpos = Z0 -(zm-14.5*CLHEP::cm); //        ! zpos of mother volume relative to centre of ball
  ...
  fRadius=2*CLHEP::cm;
  // G4cout<<"Target centre "<<(zpos+zm-0.75*CLHEP::cm-13*CLHEP::cm-0.5*CLHEP::cm)/cm<<"cm"<<G4endl;
  //  fCenter.set(0,0,zpos+zm-0.75*CLHEP::cm-13*CLHEP::cm-0.5*CLHEP::cm);//position of cell in world volume
  G4cout<<"Target centre "<<(zm-0.75*CLHEP::cm-13*CLHEP::cm+zpos-0.25*CLHEP::cm-0.5*CLHEP::cm)/CLHEP::cm<<"cm"<<G4endl;
